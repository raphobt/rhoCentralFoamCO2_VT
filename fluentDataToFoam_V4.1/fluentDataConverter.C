...
Foam::volScalarField Foam::fluentDataConverter::convertField
(
        ...
        if (fieldIDIter() == unitNumber)
        {
            ...
            )
            {
                Info<< " internal cell zone.  Size = "
                    << mesh_.nCells() << endl;
//              result.internalField() = zoneDataIter()[0];          // does not work for OF41
//              result.internalField().field() = zoneDataIter()[0];  // also does not work for OF41
//              scalarField & resultInt = const_cast<scalarField&> (result.internalField()); // also does not work for OF41
                scalarField & resultInt = const_cast<scalarField&> (result.internalField().field());  // make non-constant reference
                resultInt = zoneDataIter()[0];  // assign to non-constant reference
            }
            else
            {
...
//                  result.boundaryField()[patchID] == zoneDataIter()[0]; // does not work for OF41
                    fvPatchField<double> & resultBou = const_cast<fvPatchField<double>&>(result.boundaryField()[patchID]); // make non-constant reference
                    resultBou == zoneDataIter()[0];  // assign to non-constant reference
                }
                else
                {
